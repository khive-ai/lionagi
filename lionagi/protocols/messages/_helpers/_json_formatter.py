...
def _extract_ts_type(spec: dict, required: bool) -> tuple[str, bool]:
        ...
        for opt in spec["anyOf"]:
            ...
            if "type" in opt:
                if opt["type"] == "array" and "items" in opt:
                    it = opt["items"]
                    parts.append(
                        f"{it.get('$ref', '').split('/')[-1] or _typescript_type(it.get('type', 'any'))}[]"
                    )
                else:
    ...
    if spec.get("type") == "array":
        ...
        it = items.get("$ref", "").split("/")[-1] or _typescript_type(
            items.get("type", "any")
        )
...
def _typescript_schema(schema: dict, indent: int = 0) -> str:
    ...
    lines = [
        _ts_field_line(n, s, n in required, indent)
...
class JsonFormatter:
    ...
    def parse(
        ...
        def _parse_one(data, f):
            ...
            if isinstance(f, dict):
                return fuzzy_validate_mapping(data, f, **fuzzy_match_params.to_dict())
            return data
