...
class Message(Temporal, Adaptable):
    ...
    def validate_content_type(
    ...
    ) -> Any:  # Changed to Pydantic v2 syntax for validator
        ...
        if role == MessageRole.TOOL_REQUEST:
            if not isinstance(v, list) or not all(
                isinstance(tc, ToolCallRequest) for tc in v
            ):
                raise ValueError(
                    "Content for TOOL_REQUEST must be a list of ToolCallRequest objects."
                )
        elif role == MessageRole.TOOL_RESPONSE:
            if not isinstance(v, ToolCallResponse):
                raise ValueError(
                    "Content for TOOL_RESPONSE must be a ToolCallResponse object."
                )
        elif role in [
            MessageRole.SYSTEM,
            MessageRole.USER,
            ...
            if not isinstance(v, (str, dict)):  # Allow dict for assistant
                raise ValueError(
                    f"Content for {role.value} must be a string or a dict."
                )
        return v
...
__all__ = ["MessageRole", "ToolCallRequest", "ToolCallResponse", "Message"]
