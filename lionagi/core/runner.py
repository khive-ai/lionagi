...
def _unwrap_exception_group(exc: BaseException) -> BaseException:
    """Return the inner exception if `exc` is a single-exception group,
    otherwise return `exc` unchanged. Used to flatten anyio TaskGroup wrapping."""
    if isinstance(exc, _EGBase) and len(exc.exceptions) == 1:
        return exc.exceptions[0]
    return exc


def _reraise_unwrapped(exc: BaseException) -> None:
    """Re-raise, unwrapping a single-exception ExceptionGroup so callers can
    catch the underlying error directly. Always raises; never returns."""
...
async def _maybe_limit(limiter: CapacityLimiter | None):
    """Acquire `limiter` if provided, else no-op. Lets call sites avoid an
    if/else duplication when concurrency capping is optional."""
    if limiter is None:
        yield
        return
    async with limiter:
...
class Runner:
    ...
    def _install_observers(self) -> None:
        """Subscribe forwarding handlers that lift bus events into IPU observations.

        Each handler builds the payload appropriate to its event shape, then
        defers to the IPU. Handlers are kept in `self._handlers` as strong
        refs because EventBus subscribes via weakrefs.
        """

        async def on_start(br: Principal, node: OpNode) -> None:
            await self._observe(br, "node.start", {
                "node": str(node.id), "morphism": node.m.name,
            })

        async def on_finish(
            br: Principal, node: OpNode, result: dict[str, Any]
        ) -> None:
            await self._observe(br, "node.finish", {
                "node": str(node.id), "keys": list(result.keys()),
            })

        async def on_control(
            br: Principal, node: OpNode, action: dict[str, Any]
        ) -> None:
            await self._observe(br, "control.action", {
                "node": str(node.id),
        ...
        async def on_spawn(
            ...
            await self._observe(br, "graph.spawn", info)

        for event, handler in (
            ("node.start", on_start),
            ("node.finish", on_finish),
            ("control.action", on_control),
            ("graph.spawn", on_spawn),
        ):
            self._handlers.append(handler)
            self.bus.subscribe(event, handler)

    async def _observe(
        self, br: Principal, what: str, payload: dict[str, Any]
    ) -> None:
        await self.ipu.on_observation(
            Observation(who=br.id, what=what, payload=payload)
        )
    ...
    async def _run_parallel_nodes(
        ...
        async def _run_one(node: OpNode) -> None:
            async with _maybe_limit(limiter):
                await self._exec_node(
                    br,
                    node,
                    results,
