...
class iModel:
    ...
    def create_api_calling(
        ...
        if (self.endpoint.config.provider == "claude_code" and 
            "resume" not in kwargs and 
            "session_id" not in kwargs and
            self._provider_metadata.get("session_id")):
    ...
    async def invoke(
        ...
        try:
            ...
            if self.endpoint.config.provider == "claude_code" and completed_call and completed_call.response:
                response = completed_call.response
                if isinstance(response, dict) and "session_id" in response:
                    self._provider_metadata["session_id"] = response["session_id"]
