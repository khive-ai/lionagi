...
async def react_stream(
    ...
    analysis = await _safe_round(params, ctx, first_prompt, ReActAnalysis)
    yield analysis
...
async def _safe_round(
    ...
    """Run an intermediate round; on failure, synthesize a ReActAnalysis
    that records the error in `analysis` and asks for another round.

    Only ReActAnalysis has a clean fallback shape; other request_models
    ...
    try:
        ...
        if request_model is ReActAnalysis:
            return ReActAnalysis(
                analysis=f"Previous round failed: {e}",
...
async def _run_round(
    ...
    operate_params = OperateParams(
        operable=Operable.from_structure(request_model),
        validator=params.validator,
        generate_params=params.generate_params.with_updates(
            copy_containers="deep", primary=instruction
        ),
        invoke_actions=(
            params.invoke_actions if invoke_actions is None else invoke_actions
        ),
...
def _needs_extension(analysis: Any) -> bool:
    ...
    return bool(getattr(analysis, "extension_needed", False))
